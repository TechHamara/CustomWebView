...
@DesignerComponent(version = 7, versionName = "1.0", description = "An advanced WebView component with support for ad-blocking, deep linking, file uploads, printing, custom user agents, cookie management, desktop mode, zoom controls, and JavaScript interaction. Developed by Sunny Gupta and Customize by TechHamara.<br><a href='https://github.com/TechHamara/Th_Free_Extensions' target='_blank'><small><u>Find More Extension</u></small></a><br><a href='https://github.com/TechHamara/Th_Extensions_List/blob/main/LICENSE.md#terms-and-conditions-for-the-extension' target='_blank'><small><u>Terms & Conditions</u></small></a><br><a href='https://buymeacoffee.com/techhamara' target='_blank'><small><u>Find More On BuyMeCoffee Page</u></small></a><br><a href='https://github.com/vknow360/CustomWebView' target='_blank'><small><u>Find CustomWebView On GitHub Page</u></small></a>", iconName = "icon.png")
public final class CustomWebView extends AndroidNonvisibleComponent implements WView.SwipeCallback {
  ...
  public void CreateShortcut(String url, String iconPath, String title) {
    try {
      ...
      if (img != null) {
        ...
        String clsName = Objects
            .requireNonNull(context.getPackageManager()
                .resolveActivity(context.getPackageManager().getLaunchIntentForPackage(pkg), 0)).activityInfo.name
            .replaceAll("Screen1",
  ...
  private class WebClient extends WebViewClient {
    ...
    public WebResourceResponse shouldInterceptRequest(WebView view, String url) {
      ...
      if (url.startsWith("http://localhost/") || url.startsWith(ASSET_PREFIX)) {
        if (blockAds) {
          ...
          return ad ? ab.createEmptyResource() : super.shouldInterceptRequest(view, url);
        }
    ...
    public WebResourceResponse shouldInterceptRequest(WebView view, final WebResourceRequest request) {
      activity.runOnUiThread(new Runnable() {
        ...
        public void run() {
          try {
            ...
            RequestIntercepted(request.getUrl().toString(), YailDictionary.makeDictionary(headers));
          } catch (Exception e) {
      ...
      if ("localhost".equals(request.getUrl().getAuthority())
        ...
        if (blockAds) {
          ...
          return ad ? ab.createEmptyResource() : super.shouldInterceptRequest(view, request);
        }
    ...
    public void onReceivedHttpError(WebView view, WebResourceRequest request, WebResourceResponse errorResponse) {
      OnErrorReceived(getIndex(view), errorResponse.getReasonPhrase(), errorResponse.getStatusCode(),
          request.getUrl().toString());
    }

    @Override
    public void onReceivedError(WebView view, WebResourceRequest request, WebResourceError error) {
      OnErrorReceived(getIndex(view), error.getDescription().toString(), error.getErrorCode(),
          request.getUrl().toString());
    }
  ...
  private int getIndex(WebView view) {
    for (Map.Entry<Integer, WView> entry : wv.entrySet()) {
      if (entry.getValue() == view) {
        return entry.getKey();
      }
    }
    return iD; // Return current active ID if not found in map
  }
  ...
  @SimpleFunction(description = "Allows geolocation access")
  ...
  @SimpleEvent(description = "Event raised when SSL error is received")
  public void OnReceivedSslError(int errorCode) {
    EventDispatcher.dispatchEvent(this, "OnReceivedSslError", errorCode);
  }

  @SimpleFunction(description = "Proceeds or cancels SSL error")
  ...
  @SimpleEvent(description = "A new request is intercepted or recorded. RequestHeaders is a dictionary.")
  ...
  @SimpleFunction(description = "Captures a screenshot of the current WebView and saves it to the specified path.")
  ...
  @SimpleProperty(description = "Sets the mixed content mode. 0=Always Allow, 1=Never Allow, 2=Compatibility (Android 5+)")
  public void MixedContentMode(int mode) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      webView.getSettings().setMixedContentMode(mode);
    }
  }

  @SimpleProperty(description = "Returns the current mixed content mode")
  public int MixedContentMode() {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      return webView.getSettings().getMixedContentMode();
    }
    return 1; // Default to Never Allow
  }

  @DesignerProperty(editorType = PropertyTypeConstants.PROPERTY_TYPE_INTEGER, defaultValue = "0")
  @SimpleProperty(description = "Sets the force dark mode. 0=Off, 1=On, 2=Auto (Android 10+)")
  @SuppressWarnings("deprecation")
  public void ForceDarkMode(int mode) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
      webView.getSettings().setForceDark(mode);
    }
  }

  @SimpleProperty(description = "Returns the current force dark mode")
  @SuppressWarnings("deprecation")
  public int ForceDarkMode() {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
      return webView.getSettings().getForceDark();
    }
    return 0;
  }

  @SimpleFunction(description = "Scrolls to the top of the page")
  public void ScrollToTop() {
    webView.pageUp(true);
  }

  @SimpleFunction(description = "Scrolls to the bottom of the page")
  ...
  @SimpleProperty(description = "Sets whether webview is scrollable")
